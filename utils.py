...
def get_un_data(dataflow, structure, commod, country, transaction, key, start_period):
    ...
    commod_lookup = sdmx.to_pandas(metadata.codelist[commod])
    country_lookup = sdmx.to_pandas(metadata.codelist[country])
    transaction_lookup = sdmx.to_pandas(metadata.codelist[transaction])

    data = sdmx.Request('UNSD').data(resource_id=dataflow,
                                     key=key,
